...
list_rus = {1: 'АВЕИНОРСТ',
       ...
       8: 'ШЭЮ',
       10: 'ФЩЪ'}
word = input("write word on rus or eng language/введите слово на русском или английском языке: ")
...
if word[0] in eng:
    ...
    for x in word:
        for key, value in list_eng.items():
            if x.upper() in value:
                count += key
    print(f'the value of this word: {count}')
if word[0] in rus:
    count = 0
    for i in word:
        for key, value in list_rus.items():
            if i.upper() in value:
                count += key
    print(f'стоимость этого слова: {count}')